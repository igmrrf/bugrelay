...
class CompletenessChecker {
  ...
  async checkOpenAPICompleteness() {
    ...
    const openApiPath = join(this.docsPath, "api/openapi.yaml");
    ...
    try {
      ...
      const spec = load(openApiContent);
    ...
    for (const routerFile of routerFiles) {
      ...
      for (const route of routes) {
        ...
        if (!documentedPaths.includes(pathPattern)) {
          this.errors.push(
            `Undocumented endpoint: ${route.method} ${route.path}`,
          );
        } else if (
          !spec.paths[pathPattern] ||
          !spec.paths[pathPattern][methodLower]
        ) {
          this.errors.push(
            `Missing method documentation: ${route.method} ${pathPattern}`,
          );
        } else {
  ...
  checkEndpointQuality(endpoint, endpointName) {
    ...
    if (!responses["400"] && endpoint.requestBody) {
      this.warnings.push(
        `Missing 400 response for ${endpointName} with request body`,
      );
    }
  ...
  async checkModelDocumentation() {
    ...
    const schemaPath = join(this.docsPath, "models/schema.json");
    const dataModelsPath = join(this.docsPath, "models/data-models.md");

    if (!existsSync(schemaPath)) {
      this.errors.push("JSON Schema file not found");
    }

    if (!existsSync(dataModelsPath)) {
    ...
    for (const modelFile of modelFiles) {
      ...
      for (const model of models) {
        ...
        if (existsSync(dataModelsPath)) {
          const docsContent = readFileSync(dataModelsPath, "utf8");
          if (!docsContent.includes(model.name)) {
            this.warnings.push(
              `Model ${model.name} not documented in data-models.md`,
            );
          }
        }
      }
  ...
  async checkEndpointExamples() {
    ...
    const examplesDir = join(this.docsPath, "api/examples");
  ...
  async checkMCPToolDefinitions() {
    ...
    const mcpToolsPath = join(this.docsPath, "mcp/tools.json");
    ...
    try {
      const toolsContent = readFileSync(mcpToolsPath, "utf8");
  ...
  async checkVersionSync() {
    ...
    const backendModPath = join(this.backendPath, "go.mod");
    const docsPackagePath = join(this.docsPath, "package.json");
    const openApiPath = join(this.docsPath, "api/openapi.yaml");
    ...
    try {
      ...
      if (existsSync(docsPackagePath)) {
        const packageJson = JSON.parse(readFileSync(docsPackagePath, "utf8"));
      ...
      if (uniqueVersions.length > 1) {
        this.warnings.push(
          `Version mismatch detected: ${JSON.stringify(versions)}`,
        );
      }
    } catch (error) {
  ...
  async findRouterFiles() {
    const routerDir = join(this.backendPath, "internal/router");
    ...
    return files
      ...
      .map((file) => join(routerDir, file));
  }

  async findModelFiles() {
    const modelsDir = join(this.backendPath, "internal/models");
