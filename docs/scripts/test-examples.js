...
class ExamplesTester {
  ...
  async testCurlExamples() {
    ...
    const curlFile = join(__dirname, "../api/examples/curl-examples.md");
    if (!existsSync(curlFile)) {
      this.errors.push("curl-examples.md not found");
      return;
    }

    const content = readFileSync(curlFile, "utf8");
  ...
  async testJavaScriptExamples() {
    ...
    const jsFile = join(__dirname, "../api/examples/javascript-examples.md");
...
`;
  ...
  async testPythonExamples() {
    ...
    const pythonFile = join(__dirname, "../api/examples/python-examples.md");
    if (!existsSync(pythonFile)) {
      this.errors.push("python-examples.md not found");
      return;
    }

    const content = readFileSync(pythonFile, "utf8");
...
`;
  ...
  reportResults() {
    ...
    const reportPath = join(__dirname, "../test-reports/examples.json");
