...
class EnhancedExampleTester {
    ...
    for (const [index, example] of examples.entries()) {
      ...
      try {
        ...
        try {
          ...
          this.warnings.push(`${testName}: Python3 not available, skipping syntax check`);
          continue;
        }
  ...
  async testExampleCompleteness() {
    ...
    if (!this.openApiSpec) {
      this.warnings.push('OpenAPI spec not available, skipping completeness check');
      return;
    }
  ...
  async validateResponseExamples(filePath) {
    ...
    for (const [index, response] of jsonResponses.entries()) {
      ...
      try {
        ...
        this.errors.push(`${testName}: Invalid JSON response - ${error.message}`);
        this.testResults.push({
          test: testName,
          type: 'response-validation',
          status: 'failed',
  ...
  checkErrorHandlingPatterns(content, filePath) {
    ...
    switch (language) {
      ...
      case 'curl':
        return content.includes('400') || content.includes('401') || content.includes('error');
      default:
        return false;
    }
  }
...
`;
  ...
  reportResults() {
    ...
    const reportPath = path.join(__dirname, '../test-reports/example-accuracy.json');
...
module.exports = EnhancedExampleTester;