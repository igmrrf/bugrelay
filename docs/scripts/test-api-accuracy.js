...
class APIAccuracyTester {
    ...
    try {
      ...
      if (error.response?.status === 409) {
        ...
        try {
          ...
          const response = await axios.post(`${this.baseURL}/api/v1/auth/login`, loginData);
          this.authToken = response.data.access_token;
  ...
  async testEndpoint(pathPattern, method, endpointSpec) {
    ...
    try {
      ...
      if (!expectedStatuses.includes(response.status.toString())) {
        this.warnings.push(`${testName}: Unexpected status ${response.status}, expected one of: ${expectedStatuses.join(', ')}`);
      }
      ...
      if (error.response) {
        ...
        if (expectedStatuses.includes(status.toString())) {
          ...
          await this.validateResponseSchema(testName, error.response, endpointSpec);
  ...
  async validateResponseSchema(testName, response, endpointSpec) {
    ...
    try {
      this.validateDataAgainstSchema(response.data, schema, `${testName} response`);
    } catch (error) {
      this.warnings.push(`${testName}: Response schema validation failed - ${error.message}`);
    }
  }
    ...
    if (schema.type === 'object' && schema.properties) {
      ...
      for (const [propName, propValue] of Object.entries(data)) {
        if (schema.properties[propName]) {
          try {
            this.validateDataAgainstSchema(propValue, schema.properties[propName], `${context}.${propName}`);
          } catch (error) {
    ...
    for (const testCase of testCases) {
      try {
        ...
        if (response.status !== testCase.expectedStatus) {
          this.warnings.push(`Validation test failed for ${testCase.endpoint}: expected ${testCase.expectedStatus}, got ${response.status}`);
        } else {
          console.log(`✓ Validation working for ${testCase.endpoint}`);
        }
      } catch (error) {
        this.warnings.push(`Validation test error for ${testCase.endpoint}: ${error.message}`);
      }
    }
  }
  ...
  reportResults() {
    ...
    const reportPath = path.join(__dirname, '../test-reports/api-accuracy.json');
...
module.exports = APIAccuracyTester;