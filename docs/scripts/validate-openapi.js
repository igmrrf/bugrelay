...
class EnhancedOpenAPIValidator {
  ...
  async loadAndValidateSpec() {
    const apiPath = join(__dirname, "../api/openapi.yaml");
  ...
  validateResponses(responses, operationId) {
    ...
    const hasSuccessResponse = statusCodes.some(
    ...
    for (const [statusCode, response] of Object.entries(responses)) {
      if (!response.description) {
        this.errors.push(
          `Missing description for ${statusCode} response in ${operationId}`,
        );
      }
  ...
  validateParameters(parameters, operationId) {
    for (const [index, param] of parameters.entries()) {
      ...
      if (!param.in) {
        this.errors.push(
          `Parameter ${param.name || index} missing 'in' property in ${operationId}`,
        );
      }

      if (!param.description) {
        this.warnings.push(
          `Parameter ${param.name || index} missing description in ${operationId}`,
        );
      }

      if (!param.schema && !param.content) {
        this.errors.push(
          `Parameter ${param.name || index} missing schema in ${operationId}`,
        );
      }
    }
  }
  ...
  async validateBestPractices() {
    ...
    if (this.spec.info?.version) {
      ...
      if (!version.match(/^\d+\.\d+\.\d+$/)) {
        this.warnings.push(
          "API version should follow semantic versioning (x.y.z)",
        );
      }
    }
      ...
      for (const path of paths) {
        ...
        for (const segment of pathSegments) {
          if (segment !== segment.toLowerCase()) {
            this.warnings.push(
              `Path segment should be lowercase: ${segment} in ${path}`,
            );
          }

          if (segment.includes("_")) {
            this.warnings.push(
              `Path segment should use kebab-case: ${segment} in ${path}`,
            );
          }
        }
      }
    ...
    if (this.spec.paths) {
      for (const [path, pathItem] of Object.entries(this.spec.paths)) {
        if (pathItem.get?.requestBody) {
          this.warnings.push(
            `GET request should not have request body: ${path}`,
          );
        }

        if (pathItem.delete?.requestBody) {
          this.warnings.push(
            `DELETE request should not have request body: ${path}`,
          );
        }
      }
    }
  ...
  async validateSecurity() {
    ...
    const hasOAuthPaths = Object.keys(this.spec.paths || {}).some(
      (path) => path.includes("oauth") || path.includes("auth"),
    );

    if (hasOAuthPaths) {
      const hasOAuth2 = Object.values(securitySchemes).some(
        (scheme) => scheme.type === "oauth2",
      );

      if (!hasOAuth2) {
        this.warnings.push(
          "OAuth paths found but no OAuth2 security scheme defined",
        );
      }
    }
          ...
          if (["get", "post", "put", "patch", "delete"].includes(method)) {
            if (!operation.security && !this.spec.security) {
              ...
              if (!path.includes("public") && !path.includes("health")) {
                this.warnings.push(
                  `No security defined for ${method.toUpperCase()} ${path}`,
                );
              }
            }
          }
  ...
  async validateExamples() {
      ...
      for (const [method, operation] of Object.entries(pathItem)) {
        if (["get", "post", "put", "patch", "delete"].includes(method)) {
          ...
          if (operation.requestBody?.content) {
            ...
            ).some((content) => content.example || content.examples);
          ...
          if (operation.responses) {
            for (const [statusCode, response] of Object.entries(
              ...
              if (response.content) {
                const hasResponseExample = Object.values(response.content).some(
                  (content) => content.example || content.examples,
                );

                if (!hasResponseExample && statusCode.startsWith("2")) {
                  missingExamples++;
                  this.warnings.push(
                    `Missing response example for ${operationId} ${statusCode}`,
                  );
    ...
    console.log(
      `Found ${exampleCount} examples, ${missingExamples} missing examples`,
    );
  }
  ...
  reportResults() {
    ...
    if (this.errors.length === 0 && this.warnings.length === 0) {
      console.log(
        "✅ OpenAPI specification is comprehensive and follows best practices!",
      );
      ...
      if (this.spec) {
        ...
        console.log(
          `  Security Schemes: ${Object.keys(this.spec.components?.securitySchemes || {}).length}`,
        );
      }
    ...
    const report = {
      ...
      spec: this.spec
        ? {
            ...
            schemaCount: Object.keys(this.spec.components?.schemas || {})
              .length,
          }
        : null,
    };
    ...
    const reportPath = join(
      __dirname,
      "../test-reports/openapi-validation.json",
    );
