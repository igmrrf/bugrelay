...
class VersionSynchronizer {
  ...
  async detectVersions() {
    ...
    const packagePath = join(this.docsPath, "package.json");
  ...
  async getBackendVersion() {
    ...
    const mainGoPath = join(this.backendPath, "main.go");
    ...
    const goModPath = join(this.backendPath, "go.mod");
  ...
  async updateOpenAPIVersion(version) {
    ...
    if (spec.info.version !== version) {
      ...
      const updatedContent = dump(spec, {
        indent: 2,
        lineWidth: 120,
        noRefs: true,
      });

      writeFileSync(openApiPath, updatedContent);
  ...
  async updateMCPVersion(version) {
    const mcpMetadataPath = join(this.docsPath, "mcp/metadata.json");
    if (!existsSync(mcpMetadataPath)) {
      return;
    }

    const metadata = JSON.parse(readFileSync(mcpMetadataPath, "utf8"));

    if (metadata.version !== version) {
      metadata.version = version;
      writeFileSync(mcpMetadataPath, JSON.stringify(metadata, null, 2) + "\n");
  ...
  async updateSiteConfig(version) {
    const configPath = join(this.docsPath, ".vitepress/config.js");
