...
const APIAccuracyTester = require('./test-api-accuracy');
const ExamplesTester = require('./test-examples');
const SchemaValidator = require('./validate-schemas');
const CompletenessChecker = require('./check-completeness');
...
class ComprehensiveTestSuite {
      ...
      for (const link of links) {
        ...
        if (!fs.existsSync(targetPath)) {
          this.results.linkValidation.errors.push(
            `Broken internal link in ${path.relative(docsDir, file)}: ${link}`
          );
        }
      }
    }
      ...
      for (const link of links) {
        if (!this.isValidURL(link)) {
          this.results.linkValidation.warnings.push(
            `Potentially invalid external link in ${path.relative(docsDir, file)}: ${link}`
          );
        }
      }
    }
  ...
  async runAPIAccuracyTest() {
    ...
    if (!await this.isAPIAvailable()) {
      ...
      this.results.apiAccuracy.warnings.push('API not available, skipping accuracy tests');
      console.log('⚠️  API not available, skipping accuracy tests');
      return;
    }
  ...
  generateFinalReport() {
    ...
    for (const [key, result] of Object.entries(this.results)) {
      ...
      let statusIcon;
      switch (status) {
        case 'passed': statusIcon = '✅'; totalPassed++; break;
        case 'failed': statusIcon = '❌'; totalFailed++; break;
        case 'skipped': statusIcon = '⚠️ '; totalSkipped++; break;
    ...
    const reportPath = path.join(this.reportDir, 'comprehensive-test-report.json');
