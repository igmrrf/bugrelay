...
export const useAuthStore = create<AuthState>()(
  persist(
    (set, get) => ({
      ...
      // Computed getters
      isLoading: () => {
        return loadingManager.isLoading('auth.login') || 
      ...
      isCompanyMember: (companyId: string) => {
        const { user } = get()
        return user?.companyMemberships.some(m => m.companyId === companyId) || false
      },

      isCompanyAdmin: (companyId: string) => {
        const { user } = get()
        return user?.companyMemberships.some(m => 
          m.companyId === companyId && m.role === 'admin'
        ) || false
      },
    }),
    {
