  ...
  return useMutation({
    ...
    onSuccess: (updatedBug) => {
      ...
      queryClient.setQueryData(
        queryKeys.bugs.detail(updatedBug.id),
        updatedBug,
      );
...
export const useVoteBug = () => {
  ...
  return useMutation({
    ...
    onMutate: async (bugId: string) => {
      ...
      const previousBug = queryClient.getQueryData(
        queryKeys.bugs.detail(bugId),
      );
      ...
      queryClient.setQueryData(queryKeys.bugs.detail(bugId), (old: any) =>
        old
          ? {
              ...
              hasUserVoted: data.hasUserVoted,
            }
          : old,
      );
    },
  ...
  return useMutation({
    ...
    onSuccess: (comment, { bugId }) => {
      ...
      queryClient.invalidateQueries({
        queryKey: queryKeys.bugs.comments(bugId),
      });
...
export const useReplyToComment = () => {
  ...
  return useMutation({
    mutationFn: ({
      ...
      content: string;
    }) => bugsAPI.replyToComment(bugId, parentCommentId, content),
    onMutate: () => {
      loadingManager.start("bugs.comments.reply");
    },
    onSuccess: (comment, { bugId }) => {
      queryClient.invalidateQueries({
        queryKey: queryKeys.bugs.comments(bugId),
    ...
    onError: (error) => {
      errorHandler.handle(error, "reply to comment");
    },
    onSettled: () => {
      loadingManager.stop("bugs.comments.reply");
    },
  });
};
...
export const useUpdateComment = () => {
  ...
  return useMutation({
    mutationFn: ({
      ...
      commentId: string;
      content: string;
    }) => bugsAPI.updateComment(bugId, commentId, content),
    onMutate: () => {
      loadingManager.start("bugs.comments.update");
    },
    onSuccess: (comment, { bugId }) => {
      queryClient.invalidateQueries({
        queryKey: queryKeys.bugs.comments(bugId),
    ...
    onError: (error) => {
      errorHandler.handle(error, "update comment");
    },
    onSettled: () => {
      loadingManager.stop("bugs.comments.update");
    },
  });
};
...
export const useDeleteComment = () => {
  ...
  return useMutation({
    mutationFn: ({ bugId, commentId }: { bugId: string; commentId: string }) =>
      bugsAPI.deleteComment(bugId, commentId),
    onMutate: () => {
      loadingManager.start("bugs.comments.delete");
    },
    onSuccess: (_, { bugId }) => {
      queryClient.invalidateQueries({
        queryKey: queryKeys.bugs.comments(bugId),
    ...
    onError: (error) => {
      errorHandler.handle(error, "delete comment");
    },
    onSettled: () => {
      loadingManager.stop("bugs.comments.delete");
    },
  });
};
...
export const useFlagComment = () => {
  ...
  return useMutation({
    mutationFn: ({
      ...
      commentId: string;
      reason: string;
    }) => bugsAPI.flagComment(bugId, commentId, reason),
    onSuccess: () => {
      addToast({
    ...
    onError: (error) => {
      errorHandler.handle(error, "flag comment");
    },
  });
};

export const useSearchDuplicates = () => {
  return useMutation({
    mutationFn: (query: string) => bugsAPI.searchDuplicates(query),
    onError: (error) => {
      errorHandler.handle(error, "search for duplicates");
    },
  });
};

export const useFlagBug = () => {
  const { addToast } = useUIStore();

  return useMutation({
    mutationFn: ({ bugId, reason }: { bugId: string; reason: string }) =>
      bugsAPI.flag(bugId, reason),
    onSuccess: () => {
      addToast({
        title: "Bug flagged",
    ...
    onError: (error) => {
      errorHandler.handle(error, "flag bug");
    },
  });
};
...
export const useSearchCompanies = () => {
  return useMutation({
    mutationFn: (query: string) => bugsAPI.searchCompanies(query),
    onError: (error) => {
      errorHandler.handle(error, "search companies");
    },
  });
};

export const useGetComments = (bugId: string) => {
  return useQuery({
    queryKey: queryKeys.bugs.comments(bugId),
